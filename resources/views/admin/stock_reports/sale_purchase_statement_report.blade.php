        ...
        <div >
            <table width="100%" cellspacing="0" cellpadding="0"  align="center" style="border-collapse:collapse; margin-top: 20px" >
                ...
                @foreach($entry_dates as $entry_date )
                    ...
                    <?php
                    $memo_infos = \Illuminate\Support\Facades\DB::table('memo_account')->select('memo_account.memo_no','memo_account.id','memo_account.discount','memo_account.vat','memo_account.tax','memo_account.due', 'clients.client_name')->where('entry_date',$entry_date->entry_date)
                        ->join('clients', 'clients.id', '=', 'memo_account.client_id')->get();
                    ?>

                    @foreach($memo_infos as $memo_info)
                        <tr>
                            <td colspan="9"><b>DATE: {{date('d/m/Y', $entry_date->entry_date)}}     ----    PARTY NAME: {{$memo_info->client_name}}     ----    BILL NO: {{$memo_info->memo_no}}     ----    @if($memo_info->discount != 0)----    Discount: {{$memo_info->discount}}@endif     @if($memo_info->tax != 0)----    Tax: {{$memo_info->tax}}@endif     @if($memo_info->vat!=0)----    Vat: {{$memo_info->vat}}@endif     ----    Total: {{$memo_info->due}}</b></td>
                        </tr>
                        ...
                        $grand_amount += str_replace(',', '', $memo_info->due);
                        ?>
                    @endforeach
